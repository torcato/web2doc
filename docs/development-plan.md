...
All phases are initially pending. A phase is complete when its deliverables and exit criteria are demonstrated, not merely when its tasks are implemented.

## Python library adoption plan

The architecture already selects the core stack. This section makes those decisions actionable within the development phases. Add dependencies when their phase begins; packages listed here are planned and have not yet been installed or compatibility-tested together.
...
These are evaluation options, not dependencies to install together. The [architecture alternatives](architecture-and-development-plan.md#alternatives-and-adoption-triggers) and [case-study tool comparison](website-documentation-case-study.md#tool-comparison) contain the supporting rationale and sources.
