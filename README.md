...
Resume an unguided run that stopped because a time, action, state, model-call, or token budget was exhausted, or after
authentication became invalid. Reauthenticate first when required. The additional limits apply only to the resumed
invocation; existing evidence, features, and pending frontier items remain on the same run:

```bash
uv run web2doc discover-resume demo DISCOVERY_RUN_ID \
  --additional-seconds 3600 \
  --additional-actions 500 \
  --additional-states 500
```
...
```bash
uv run web2doc docs-generate demo ./published-documentation-v2 \
  --resume-run DISCOVERY_RUN_ID \
  --max-seconds 3600 \
  --max-actions 500 \
  --max-states 500
```
...
For applications with large navigation menus, raise the per-page candidate limit with
`--max-candidates-per-state 200` (the same option is available on `discover`).
